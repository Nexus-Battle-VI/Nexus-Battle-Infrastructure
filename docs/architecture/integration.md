...
| Catalog → Notifications (`suspended`/`reactivated`/`inventory.adjusted`/`premium.configured`, HU-38) | Asíncrono | Consumidor implementado en Notifications (#20/#21/#22); Catalog ya escribe los 4 eventos en su outbox, pero **no tiene ningún dispatcher/worker que los publique** (auditado en código: no existe); sin cola SQS provisionada. ADR-017 `Accepted` cubre explícitamente solo `catalog.product.created`; [ADR-018](../adr/ADR-018-catalog-lifecycle-events-transport.md) (`Proposed`, no `Accepted`) propone una cola compartida para los 4, pero no se provisiona nada de ella en este cambio. Auditado además: aunque ADR-018 se aceptara, el `lifecycleQueue` de Notifications sigue acoplado a `QUEUE_DRIVER` general y a la DLQ general -mismo defecto que #23 corrigió para `created`, sin corregir aquí-, así que activarla exigiría un PR de Notifications adicional. No desplegado -brecha de Catalog y de una decisión arquitectónica sin aceptar, no de esta Task- |
