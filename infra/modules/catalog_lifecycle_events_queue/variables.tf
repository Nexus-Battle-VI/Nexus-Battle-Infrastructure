variable "environment" {
  description = "Entorno de despliegue. Forma parte del nombre de la cola, igual que en infra/modules/catalog_events_queue (ADR-017)."
  type        = string
...
variable "max_receive_count" {
  description = "Intentos antes de mover el mensaje a la DLQ (5 por ADR-018, mismo valor que ADR-017)."
  type        = number
  default     = 5
}
...
variable "max_age_oldest_message_alarm_seconds" {
  description = <<-DESC
    ...
    la cola principal. 600 s, mismo criterio que infra/modules/catalog_events_queue:
    diez veces el visibility timeout de 60 s de ADR-018.
  DESC
  type        = number
  default     = 600
}
